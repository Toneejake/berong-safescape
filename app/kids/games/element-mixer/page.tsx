    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-purple-900 via-slate-900 to-slate-950">
            ...
            <div className="fixed top-0 left-0 right-0 z-50 bg-black/70 backdrop-blur-sm px-4 py-3 flex items-center justify-between">
                ...
                <h1 className="text-white font-bold text-lg hidden sm:block">🧪 Element Mixer Lab</h1>
                <Button
                    variant="ghost"
            ...
            {/* Loading overlay */}
            {isLoading && (
                <div className="fixed inset-0 z-40 flex items-center justify-center bg-gradient-to-b from-purple-900 via-slate-900 to-slate-950">
                    <div className="text-center">
                        <Loader2 className="h-16 w-16 animate-spin text-purple-400 mx-auto mb-4" />
                        <p className="text-white text-xl font-semibold">Loading Element Mixer Lab...</p>
                        <p className="text-purple-300 mt-2">Learn about the Fire Triangle! 🔥</p>
                    </div>
                </div>
            )}
            ...
            {/* Game iframe */}
            <iframe
                src="/games/element-mixer/index.html"
                className="fixed inset-0 w-full h-full pt-14 border-0"
                onLoad={() => setIsLoading(false)}
                allow="fullscreen; autoplay"
                title="Element Mixer Lab Game"
            />
        </div>
    )
}
